define([
	...
	'esri/PopupTemplate',
	'dojo/topic'
], function (declare, lang, LocateButton, SimpleRenderer, PictureMarkerSymbol, GraphicsLayer, PopupTemplate, topic) {
	return declare(null, {
		...
		startup: function () {
			...
			this.options.map.add(this.gLayer);
			this.options.graphicsLayer = this.gLayer;
			this.options.popupTemplate = new PopupTemplate({ title: 'GPS Position', content: '{*}' });
			this.options.locationSymbol = null;
		...
		_growlLocation: function (evt) {
			...
			if (this.gLayer.graphics.length > 0) {
				this.gLayer.graphics.attributes = stats;
			}

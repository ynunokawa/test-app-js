define([
	'esri/Map', // 4.0
	'esri/views/MapView', // 4.0
	'esri/views/SceneView', // 4.0
	'dojo/dom',
	'dojo/dom-style',
	'dojo/dom-geometry',
	'dojo/dom-class',
	...
	'dojo/has',
	'dojo/topic',
	//'esri/dijit/PopupMobile', 未実装。代わりに Popup を使用
	'esri/widgets/Popup',
	'dijit/Menu'//,
	//'esri/IdentityManager' 未実装
], function (Map, MapView, SceneView, dom, domStyle, domGeom, domClass, on, array, BorderContainer, ContentPane, FloatingTitlePane, lang, mapOverlay, FloatingWidgetDialog, put, aspect, has, topic, Popup, Menu) {
	...
	return {
		...
		initMap: function () {
			if (has('phone') && !this.config.mapOptions.infoWindow) {
				this.config.mapOptions.popup = new Popup(null, put('div'));
			}
			this.map = new Map(this.config.mapOptions);
			this.map.then(lang.hitch(this, this.initLayers), function(error){ console.log("The map's resources failed to load: ", error); });
			if(this.config.viewType === '2d') {
				this.config.viewOptions2D.map = this.map;
				this.view = new MapView(this.config.viewOptions2D);
				this.view.height = 'auto';
				this.view.width = 'auto';
			} else if(this.config.viewType === '3d') {
				this.config.viewOptions3D.map = this.map;
				this.view = new SceneView(this.config.viewOptions3D);
				this.view.height = 'auto';
				this.view.width = 'auto';
			} else {
				console.error('viewType should be 2d or 3d.');
			}
			if (this.config.operationalLayers && this.config.operationalLayers.length > 0) {
				on.once(this.map, 'layer-add', lang.hitch(this, 'initWidgets'));
			} else {
				this.initWidgets();
			}
		},
